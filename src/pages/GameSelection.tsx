        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-4 gap-6 lg:gap-8">
            {games.map((game) => (
              <Card
                key={game.id}
                className={`relative overflow-hidden transition-all duration-300 hover:scale-105 hover:shadow-xl cursor-pointer bg-gradient-to-br ${game.bgGradient} border-2 border-white/50 shadow-lg ${
              ...
              >
                <div className="p-6 text-center h-full flex flex-col justify-between min-h-[200px]">
                  ...
                  <div className="space-y-3">
                    <h3 className={`text-lg sm:text-xl font-bold ${game.color}`}>
                      {game.name}
                    </h3>
                    <p className={`text-sm ${game.color.replace('700', '600')} leading-relaxed`}>
                      {game.description}
                    </p>
                  </div>
                  ...
                  <div className="mt-4">
                    {game.isImplemented ? (
                      <div className="inline-flex items-center gap-1 bg-green-500/20 backdrop-blur-sm rounded-full px-3 py-1 border border-green-300">
                        ...
                        <span className="text-xs font-semibold text-green-700">Ready to Play</span>
                      </div>
                    ) : (
